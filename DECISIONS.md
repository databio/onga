...
| Terms | Enum | Reason kept atomic |
|---|---|---|
| `conservative IDR thresholded peaks`, `optimal IDR thresholded peaks`, `representative IDR thresholded peaks`, `pseudoreplicated IDR thresholded peaks`, `replicated peaks`, `pseudoreplicated peaks`, `representative DNase hypersensitivity sites`, `consensus DNase hypersensitivity sites` | DataType | Reproducibility-selection axis. Deferred this session; only cross-cuts ~2 base families (peaks, DHS), below the payoff bar. (DEC-0001, DEC-0002, DEC-0003, DEC-0004, DEC-0005, DEC-0006, DEC-0007, DEC-0008) |
| `peaks and background as input for IDR`, `IDR ranked peaks`, `ranked gRNAs` | DataType | IDR input / ranking axis. A role/input and a scoring output, not a threshold cut (ranked ≠ thresholded). (DEC-0009, DEC-0010, DEC-0011) |
| `bias-corrected predicted signal profile` | DataType | Bias-correction axis. Only touches the signal/bias-profile family (~1–2 bases); doesn't cross-cut; deferred for a future `bias_correction` facet. See note below. (DEC-0012) |
| `redacted alignments`, `redacted transcriptome alignments` | DataType | Redaction axis. 2 terms, single (alignment) base family; kept atomic. (DEC-0013, DEC-0014) |
| `wavelet-smoothed signal`, `summed densities signal`, `smoothed methylation state at CpG` | DataType, FeatureType | Smoothing axis. A transform, not scaling; deferred. (DEC-0015, DEC-0016, DEC-0017) |
| `selected regions for predicted signal profile`, `selected regions for bias-corrected predicted signal profile`, `selected regions for predicted bias profile`, `selected regions for count sequence contribution scores`, `selected regions for predicted signal and sequence contribution scores`, `selected regions for profile sequence contribution scores` | DataType | Selected-regions wrapper axis. A role/geometry wrapper, not an orthogonal content axis; deferred. (DEC-0018, DEC-0019, DEC-0020, DEC-0021, DEC-0022, DEC-0023) |
| `DNN-MPRA predicted signal`, `HMM predicted chromatin state` | DataType | Assay/method-fused predicted axis. The model/assay name is part of identity, not an orthogonal facet; kept atomic (the assay-name axis is itself deferred — see principle #5). (DEC-0024, DEC-0025) |
| `diploid personal genome alignments` | DataType | Reference ploidy axis. `diploid` qualifies the REFERENCE (its ploidy), not the haplotype resolution of the content; not a value of `HaplotypeResolution` (operation #17). Kept atomic. (DEC-0026) |
| `allele-specific variants`, `maternal variant calls`, `paternal variant calls`, `maternal haplotype mapping`, `paternal haplotype mapping` | FeatureType | Parental origin + allelic-imbalance sense axis. `allele-specific variants` is the ALLELIC-IMBALANCE behavior (a different sense of "allele-specific"), not allele-resolved content. The maternal/paternal terms carry a parental-origin axis (which parent a haplotype came from) that is SEPARATE from `HaplotypeResolution` and DEFERRED to a future parental-origin facet — collapsing them into `haplotype_specific` would lose the parent label (operation #17). (DEC-0027, DEC-0028, DEC-0029, DEC-0030, DEC-0031) |
| `rejected reads`, `filtered regions` | DataType | Leaves that must NOT be touched (sanity guard). (scripts/apply_filter_decomposition.py) (DEC-0117, DEC-0118) |
| `fold change over control`, `control normalized signal`, `signal p-value`, `enrichment`, `z scores matrix`, `fold over change matrix`, `raw data`, `raw imaging signal`, `signal`, `end position signal` | DataType | Terms that must NOT be touched (sanity guard). Statistic-type output_types and smoothing terms (distinct output_types, not scaling) plus the bare/profile bases and raw-data leaves. (scripts/apply_normalization_decomposition.py) (DEC-0121, DEC-0122, DEC-0123, DEC-0124, DEC-0125, DEC-0126, DEC-0127, DEC-0128, DEC-0129, DEC-0130) |
| `element gene links` | FeatureType | The atomic base `element gene links` must survive untouched. (scripts/apply_thresholding_featuretype.py) (DEC-0132) |
| `negative control regions`, `positive control regions`, `predicted enhancers`, `predicted 3D structural ensembles` | DataType, FeatureType | Terms that must NOT be touched (deferred / out of scope). Confirmed intact after the run. `control` is excluded from derivation, so control-role terms stay; bias-corrected / selected-regions / enhancer / assay-fused predicted / 3D-ensemble cases are deferred. (scripts/apply_derivation_decomposition.py) (DEC-0137, DEC-0138, DEC-0139, DEC-0140) |
| `methylation state at CpG`, `transcription start sites`, `transcribed fragments` | FeatureType | Terms that must NOT be touched (sanity guard). The smoothing sibling shares a base but is a separate axis; the three bases that already exist must survive. (scripts/apply_feature_strand_filter.py) (DEC-0143, DEC-0144, DEC-0145) |
